...
class STMaskGIT(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(self, config: GenieConfig):
        ...
        if config.with_act:
            ...
            self.pos_embed_actions = torch.nn.Parameter(torch.zeros(17, config.d_model))
    ...
    def maskgit_generate(
        ...
        if self.with_act:
            logits_CTHW = self.compute_logits_maskgit(prompt_THW, actions=act)
        else: 
        ...
        for step in tqdm(range(maskgit_steps)):
            ...
            if step > 0:  # recompute logits with updated prompt
                if self.with_act:
                    logits_CHW = self.compute_logits_maskgit(prompt_THW, actions=act)[:, :, out_t]
                else:
    ...
    def compute_loss_and_acc(self, logits_CTHW, targets_THW, relevant_mask_THW):
        ...
        logits_CTHW, targets_THW = logits_CTHW[:, :, self.config.num_prompt_frames:], targets_THW[:, self.config.num_prompt_frames:]
    ...
    def compute_logits_maskgit(self, x_THW, actions=None):
        # x_THW is for z0,...,zT while x_targets is z1,...,zT
        x_TS = rearrange(x_THW, "B T H W -> B T (H W)")
        x_TSC = self.token_embed(x_TS)
    ...
    def forward(self, input_ids, labels, actions=None):
        ...
        relevant_mask = x_THW[:, self.config.num_prompt_frames:] == self.mask_token_id  # could also get mask of corrupted tokens by uncommenting line in `get_maskgit_collator`
